...
class MaxBinaryHeap {
  ...
  extractMax(): number | null {
    ...
    let firstChild =
      firstChildIdx >= this.values.length ? -1 : this.values[firstChildIdx];
    let secondChild =
      secondChildIdx >= this.values.length ? -1 : this.values[secondChildIdx];

    while (
      this.values[parentIdx] < firstChild ||
      this.values[parentIdx] < secondChild
    ) {
      if (
        this.values[parentIdx] < firstChild &&
        this.values[parentIdx] < secondChild
      ...
      firstChild =
        2 * parentIdx + 1 >= this.values.length
          ? -1
          : this.values[2 * parentIdx + 1];
      secondChild =
        2 * parentIdx + 2 >= this.values.length
          ? -1
          : this.values[2 * parentIdx + 2];
    }
