...
class PriorityQueue {
  ...
  enqueue(name: string, priority: number) {
    ...
    while (parentIdx >= 0) {
      ...
      if (
        this.values[parentIdx].priority > this.values[pushedValIndex].priority
      ) {
  ...
  dequeue(): PriorityQueueNode | null {
    ...
    let firstChildPriority =
      firstChildIdx >= this.values.length
        ? Math.min()
        : this.values[firstChildIdx].priority;
    let secondChildPriority =
      secondChildIdx >= this.values.length
        ? Math.min()
        : this.values[secondChildIdx].priority;

    while (
      this.values[parentIdx].priority > firstChildPriority ||
